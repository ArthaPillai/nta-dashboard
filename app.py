...
with tabs[0]:
    ...
    fig1 = px.pie(values=request_type_counts.values, 
                  names=request_type_counts.index,
                  title='Distribution of Request Types',
                  color_discrete_sequence=['#1f77b4', '#ff7f0e', '#2ca02c'])

    fig1.update_layout(height=400, width=500, legend_title_text='Request Type')
...
with tabs[1]:
    ...
    fig2 = px.bar(law_school_analysis, 
                  x='Law_School', 
                  y='Avg_Extended_Time',
...
with tabs[2]:
    ...
    fig3 = px.bar(x=extended_time_counts.index, 
    ...
    fig3.update_layout(height=400, width=600, coloraxis_colorbar_title="No. of Requests")
...
with tabs[3]:
    ...
    fig4 = px.bar(y=diagnosis_counts.index, 
    ...
    fig4.update_layout(height=500, width=600, coloraxis_colorbar_title="No. of Cases")
    st.plotly_chart(fig4, use_container_width=True)

# 5. Requests by Law School
with tabs[4]:
    law_school_counts = df['Law_School'].value_counts()
    fig5 = px.bar(y=law_school_counts.index, 
...
with tabs[5]:
    ...
    fig6.update_layout(height=400, width=600, barmode='stack', legend_title_text='Approval Status')
    st.plotly_chart(fig6, use_container_width=True)

# 7. Approval Distribution
with tabs[6]:
    approval_counts = df['Approved?'].value_counts()
    fig7 = px.bar(x=approval_counts.index, 
    ...
    fig7.update_layout(height=400, width=500, coloraxis_colorbar_title="Approval Count")
...
with tabs[8]:
    ...
    fig9 = px.scatter(law_school_stats, 
                     x='Avg_Extended_Time', 
                     y='Approval_Rate',
                     size='Request_Count',
                     color='Law_School',
    ...
    fig9.update_traces(marker=dict(sizeref=2.*max(law_school_stats['Request_Count'])/(40.**2), sizemode='area'))
    
    
    fig9.update_traces(mode='markers+text', textposition='top center')
